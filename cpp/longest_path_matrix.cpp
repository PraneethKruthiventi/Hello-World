#include <iostream>
using namespace std;
// can move right left up down
int main()
{
	long long int n;
	cin >> n;
	long long int value[n][n];
	long long int length[n][n];

	long long temp_left, temp_up, temp_down, temp_right, temp_length = 0;
	//input
	for (int i = 0; i < n; i++)
		for (int j = 0; j < n; j++)
			cin >> value[i][j];
	...
	for (int i = 0; i < n; i++)
	{
		for (int j = 0; j < n; j++)
		{
			...
			if(i != 0 && j != 0 && i != n-1 && j != n-1)
			{
				...
				temp_right = value[i][j] - value[i][j+1]; 
				if(temp_right == 1 || temp_right == -1)
					length[i][j] = length[i][j+1] + 1;
				
				temp_down = value[i][j] - value[i+1][j]; 
